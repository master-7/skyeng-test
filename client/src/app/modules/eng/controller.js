...
export default class EngController {
	constructor($state, $cookies, $sessionStorage, $mdDialog, $mdToast, WordEngService, FailAnswerService, TestService) {
	...
	answerEventHandler ($event, id, index) {
		...
		if(!$(element).hasClass("red green")) {
			...
			this.wordEngResource.query({id: id}).$promise.then(
				(data) => {
